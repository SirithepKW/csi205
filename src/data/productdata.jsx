...
//     ProductWithPrice = productsList.map((product,...rest) => {
//         delete product.albumId
//         delete product.url
//         thumbnailUrl: `https://picsum.photos/150?random=${rest.id}`,
...
export function fetchProduct() {
  if (ProductWithPrice.length === 0) {
    ProductWithPrice = productsList.map(({ albumId, url, ...rest }) => ({
      ...rest,
      thumbnailUrl: `https://picsum.photos/150?random=${rest.id}`,
      price: Math.round(Math.random() * 10000) / 100 + 10
    }))
  }
  return ProductWithPrice
}
